...
class PdfLogic extends Logic
{
    ...
    public function assetBack($apply)
    {
        ...
        $assetGetList = AssetGetList::find()->where(['in', 'id', $assetListIds])->all();
        /**
         * @var AssetGetList $v
         */
        foreach ($assetGetList as $v) {
    ...
    public function applyTransfer($apply)
    {
        ...
        $arrInfo = [
            ...
            'old_base_salary' => \yii::$app->formatter->asCurrency($apply->applyTransfer->old_base_salary),
            'old_jixiao' => \yii::$app->formatter->asCurrency($apply->applyTransfer->old_jixiao),
            'target_org_name' => $apply->applyTransfer->target_org_name,
            'target_base_salary' => \yii::$app->formatter->asCurrency($apply->applyTransfer->target_base_salary),
            'target_jixiao' => \yii::$app->formatter->asCurrency($apply->applyTransfer->target_jixiao),
